...
public class BullCowController {
    ...
    public void sendMessage(KafkaDao msg) {
        try {
            kafkaTemplate.send("BULL-COW-OUT", objectMapper.writeValueAsString(msg));
        } catch (Exception ex){

        }
    }

    @KafkaListener(topics = "BULL-COW-IN", groupId = "bull-cow-group-micro")
    public void listenGroupFoo(String message) {
        try {
            KafkaDao2 req = objectMapper.readValue(message, KafkaDao2.class);
            String arg = req.getArg(), token = req.getToken();
            ResponseEntity<BullCowGameDto> re = null;
            switch (req.getMethod()){
                case ("getInfo"):
                    re = getInfo(token);
                    break;
                case ("setAnswer"):
                    re = setAnswer(token, arg);
                    break;
                case ("playerGuess"):
                    re = playerGuess(token, arg);
                    break;
                default:
                    break;
            }
            KafkaDao resp = new KafkaDao();
            resp.setId(req.getId());
            resp.setObject(re.getBody());
            resp.setResponseCode(re.getStatusCodeValue());
            sendMessage(resp);
        } catch (Exception ex){
